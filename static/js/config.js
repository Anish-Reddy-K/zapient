...
(function() {
    ...
    function initConfig() {
        ...
        setupFormSubmission();
        preloadTemplateData();
        resetSubmitButtonState('Create Agent'); // Initial button state
    }
     ...
     */
    function resetSubmitButtonState(text, disabled = false) {
    ...
    function addFileToUI(file) {
        ...
        removeBtn.addEventListener('click', function() {
            ...
            if (currentAgentName && processingComplete) {
                ...
                removeFile(fileName);
                fileItem.remove();
                if (uploadedFiles.length === 0) { // If no files left, reset button to Create Agent
                    resetSubmitButtonState('Create Agent');
    ...
    function startProcessingStatusMonitor(agentName) {
        ...
        processingComplete = false;
        resetSubmitButtonState('Processing Files...', true); // Button state during processing
        ...
        fetch(`/api/agents/${currentAgentName}/processing-status`)
            ...
            .then(data => {
                ...
                if (data.processing_complete && !processingComplete) {
                    ...
                    resetSubmitButtonState('Confirm Agent Creation'); // Button state after processing complete
    ...
    function setupFormSubmission() {
        agentConfigForm.addEventListener('submit', function(e) {
            ...
            resetSubmitButtonState('Creating...', true); // Button state during agent creation
            ...
            .then(data => {
                ...
                if (uploadedFiles.length > 0) {
                    ...
                    // No files to process, enable button immediately and show complete message
                    processingComplete = true; // Mark processing as complete even without files
                    resetSubmitButtonState('Confirm Agent Creation');
