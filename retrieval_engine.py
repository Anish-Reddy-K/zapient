...
# For local embeddings + semantic search
...
# You can pick any model from https://www.sbert.net/docs/pretrained_models.html
...
class ChunkData:
    """
    Holds the chunk's text, plus metadata:
    - doc_filename: which processed JSON or doc
    - page_number: from the PDF
    - chunk_index: sequence index
    """
...
class RetrievalEngine:
    """
    Builds two sets of chunks (large and small) from the processed JSONs for an agent,
    then provides:
      - semantic_search() on the large chunks
      - keyword_search() on the small chunks
    """
    ...
    def __init__(self, agent_processed_dir: str):
        ...
        self.large_embeddings = None  # Will hold a tensor of shape (#chunks, embedding_dim)
    ...
    def _build_large_and_small_chunks(self):
        """
        Reads each processed JSON in the agent's processed folder,
        extracts the text from each page, and splits into large & small chunks.
        """
        ...
        for proc_file in processed_files:
            ...
            # 'doc' should have structure with 'metadata' and 'content'
            content_list = doc.get('content', [])
            for page_data in content_list:
                page_num = page_data.get('page_number', -1)
                page_text = page_data.get('text', "")

                # Large chunks for semantic search
                ...
                for idx, chunk_text in enumerate(large_page_chunks):
                    chunk = ChunkData(
                        text=chunk_text,
                        doc_filename=proc_file,
                        page_number=page_num,
                        chunk_index=idx
                    )
                    self.large_chunks.append(chunk)

                # Small chunks for keyword search
    ...
    def _split_text_into_chunks(self, text: str, chunk_size: int, overlap: int) -> List[str]:
        """
        Very naive chunking approach: splits text by chunk_size chars, with overlap.
        You can expand this to do smarter token-based chunking if desired.
        """
        chunks = []
        start = 0
        while start < len(text):
            ...
            start += (chunk_size - overlap)  # Slide the window by chunk_size-overlap
        return chunks

    def semantic_search(self, query: str, top_k: int = TOP_K_SEMANTIC) -> List[ChunkData]:
        """
        Uses embeddings of the "large" chunks to find top_k semantically relevant chunks.
        """
        ...
        # 'scores' is a 1D tensor, one score per large chunk
        top_results = scores.topk(k=min(top_k, len(scores)), largest=True)

        # Return the top_k chunk objects, sorted by descending score
        top_indices = top_results[1].cpu().numpy().tolist()
        top_scores = top_results[0].cpu().numpy().tolist()

        result = []
        for idx, score in zip(top_indices, top_scores):
            chunk_obj = self.large_chunks[idx]
            result.append(chunk_obj)
        return result

    def keyword_search(self, query: str, top_k: int = TOP_K_KEYWORD) -> List[ChunkData]:
        """
        Ranks the "small" chunks by how many exact keyword hits they contain.
        This is a naive approach: you can make it more complex with fuzzy matching, etc.
        """
        if not self.small_chunks:
            return []

        # Simple approach: split the query into keywords by non-alphanum
        ...
        for i, chunk in enumerate(self.small_chunks):
            ...
            # e.g. sum(1 for k in keywords if k in text_lower)
            score = 0
            for k in keywords:
                if k in text_lower:
                    score += 1
...
def save_retrieval_results(
    ...
    """
    Saves the retrieval results to `retrieval_results.json` in the agent's directory.
    We'll store them keyed by conversation_id + the query.
    """
    agent_dir = os.path.join(data_dir, username, 'AGENTS', agent_name)
    retrieval_file = os.path.join(agent_dir, 'retrieval_results.json')

    # Load or create retrieval file
    ...
    # We might store retrieval results in a nested structure:
    # retrieval_data[conversation_id][query] = { "semantic": [...], "keyword": [...] }
    if conversation_id not in retrieval_data:
        retrieval_data[conversation_id] = {}

    # Convert chunk objects to dict
    semantic_list = []
